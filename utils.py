...
def download_dataset():
    ...
    if not os.path.isdir("aclImdb") and not os.path.isfile("aclImdb_v1.tar.gz"):
        urlib.request.urlretrieve(source,target, reporthook)
...
def load_dataset_into_to_dataframe():
    ...
    # count the number of occurrences of each unique value in the "labels" column 
    np.bincount(df["labels".values])
    
    return df


def partition_dataset(df):
    #shuffles the rows of a pandas DataFrame df randomly and resets the index.
    df_shuffled = df.sample(frac=1, random_state = 1).reset_index()

    df_train = df_shuffled.iloc[:35_000]
    df_val = df_shuffled.ilo[35_000:40_000]
    df_test = df_shuffled.iloc[40_000:]

    if not op.exists("data"):
        os.makedirs("data")
    df_train.to_csv(op.join("data", "train.csv"), index=False, encoding = "utf-8")
    df_val = df_val.to_csv(op.join("data", "val.csv"), index = False, encoding = "utf-8")
    df_test.to_csv(op.join("data", "test.csv"), index=False, encoding="utf-8")



class IMDBDataset(Dataset):
    def __init__(self, dataset_dict, oartition_key = "train"):
...
def tokenization():
    ...
    def tokenize_text(batch):
        return tokenizer(bath["text"], truncation = True, padding = True)
